...
```bash
# 推荐方式（自动安装 uvicorn、httpx 等标准依赖）
pip install "fastapi[standard]"

# 或手动安装各个组件
pip install fastapi uvicorn sse-starlette
# 当前最新版本：FastAPI 0.136.0, uvicorn 0.34.0, sse-starlette 2.2.1
```
...
```bash
# 推荐方式（FastAPI 内置 CLI，自动检测应用并启动）
fastapi dev api/main.py          # 开发模式（自动重载）
fastapi run api/main.py          # 生产模式
...
# 注意：旧版 on_event("startup"/"shutdown") 已弃用，应使用 lifespan
```
```

### 5.2 应用生命周期（推荐替代 on_event）

> **注意：** `@app.on_event("startup")` / `@app.on_event("shutdown")` 已在 Starlette 0.26+ 中弃用。
> 推荐使用 `lifespan` 上下文管理器：
